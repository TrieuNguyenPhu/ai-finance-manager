...
func NewConsumer(ctx context.Context, queueURL, endpoint, region, accessKey, secretKey string, visibilityTimeout int32, handler Handler) (*Consumer, error) {
	...
	return &Consumer{client: client, queueURL: queueURL, handler: handler, visibilityTimeout: visibilityTimeout, logger: log.Default()}, nil
}

func (c *Consumer) Run(ctx context.Context) {
	for ctx.Err() == nil {
		output, err := c.client.ReceiveMessage(ctx, &sqs.ReceiveMessageInput{QueueUrl: aws.String(c.queueURL), MaxNumberOfMessages: 10, WaitTimeSeconds: 20, VisibilityTimeout: c.visibilityTimeout})
		if err != nil {
			if ctx.Err() != nil {
				return
			}
